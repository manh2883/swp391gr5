...
public class ProductDAO extends DBContext {
    ...
    public ArrayList<Product> getAllProductForMarketing(String brand, String category, String status, String title, String brief, int index,String uid) {
        ArrayList<Product> proList = new ArrayList<>();
        try {
            String sql = "  select a.*, b.category from (select p.* , s.[value] as brand, substring([description],0,60) as brief from product p , setting as s \n"
                    ...
                    + "	(select p.product_id, s.[value] as category from product p , setting as s\n"
                    + "  where s.setting_id = p.category_id and p.category_id like ?) as b\n"
                    + " where a.product_id = b.product_id  and a.title like ? and  a.brief like ? and a.active like ? and a.author_id like  '%"+uid+"%' "
    ...
    public int getNumberProductForMarketing(String brand, String category, String status, String title, String brief,String uid) {
        try {
