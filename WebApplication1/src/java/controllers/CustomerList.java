...
public class CustomerList extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (account == null) {
            session.setAttribute("prevLink", "customerlist");
            response.sendRedirect(request.getContextPath() + "/Login");
            return;
        } else if (!perDAO.checkPermissionForRole("CustomerList", account.getRoleId())) {
            request.setAttribute("message", "No permission");
            request.getRequestDispatcher("Home/Error404.jsp").forward(request, response);
            return;
        }
        List<Object[]> customers = uDAO.getCustomerList();
